...
def getMyFeatureMaps(imageArray, convLayers):
    ...
    outputFeatureMap(imageArray, convLayers[0][0])
    outputFeatureMap(imageArray, convLayers[0][2])

    outputFeatureMap(imageArray, convLayers[1][0])
    outputFeatureMap(imageArray, convLayers[1][2])

    outputFeatureMap(imageArray, convLayers[2])
...
logits, convLayers= nn.LeNetModified(x, classesSize)
...
for i in os.listdir("./InternetImages"):
    image = cv2.imread(("./InternetImages/" + i), cv2.IMREAD_GRAYSCALE)
    ...
    channels = 1
    imageArray = np.zeros(dtype = np.float32, shape = (1,image.shape[0], image.shape[1], channels))

    imageArray[0,:,:,0] = image

    np.set_printoptions(threshold=sys.maxsize)

    imageArray = (imageArray - 128.0) / 128.0
    ...
    with tf.Session() as sess:
        ...
        plot.barPlot2(np.arange(0,classesSize,1), yVal, xLabel='Classes', setXAxis= (-1,43),
                yLabel='Logit Values [%]', fileName='Test', save=False, show=False, title = theTitle)
