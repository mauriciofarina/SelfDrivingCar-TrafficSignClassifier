...
def LeNetModified(x, image_shape, n_classes, keep_prob = None,  mu = 0 , sigma = 0.1):    
    ...
    conv3_filterShape = 1
    conv3_inputDepth = conv2_filters
    conv3_filters = 50

    conv3_W = tf.Variable(tf.truncated_normal(shape=(conv3_filterShape, conv3_filterShape, conv3_inputDepth, conv3_filters), mean = mu, stddev = sigma))
    conv3_b = tf.Variable(tf.zeros(conv3_filters))
    ...
    # SOLUTION: Layer 3: Fully Connected. Input = 400. Output = 120.
    fc1_W = tf.Variable(tf.truncated_normal(shape=(1800, 120), mean = mu, stddev = sigma))
    fc1_b = tf.Variable(tf.zeros(120))
    fc1   = tf.matmul(fc0, fc1_W) + fc1_b
